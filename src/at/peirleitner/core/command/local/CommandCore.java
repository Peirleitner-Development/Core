...
public class CommandCore implements CommandExecutor {
	...
	public boolean onCommand(CommandSender cs, Command cmd, String arg, String[] args) {
		...
		if(args.length == 0 || !cs.hasPermission(CorePermission.COMMAND_CORE_ADMIN.getPermission())) {
			...
			cs.sendMessage(Core.getInstance().getLanguageManager().getPrefix(Core.getInstance().getPluginName(), Language.ENGLISH) + this.VERSION_OUTPUT
					.replace("{0}", pdf.getName())
					.replace("{1}", pdf.getVersion())
					.replace("{2}", pdf.getAuthors().get(0))
					.replace("{3}", pdf.getWebsite())
					);
			return true;
		}
		...
		if(args.length == 1) {
			...
			if(args[0].equalsIgnoreCase("loadDefaultSaveTypes")) {
				...
				cs.sendMessage(Core.getInstance().getLanguageManager().getMessage(Core.getInstance().getPluginName(), Language.ENGLISH, "command.core.loadDefaultSaveTypes.info", null));
				return true;
			} else {
				this.sendHelp(cs);
				return true;
	...
	private final void sendHelp(@Nonnull CommandSender cs) {
		cs.sendMessage(Core.getInstance().getLanguageManager().getMessage(Core.getInstance().getPluginName(), Language.ENGLISH, "command.core.syntax", null));
	}
